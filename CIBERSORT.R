...
pdf("CIBERSORT_DGIST.pdf", width=8, height=8)
p <- ggplot(df.m, aes(fill=X, y=value, x=variable))+geom_bar( stat="identity")+geom_text(aes(label = round(value, digits=2)),size =4.5, position = position_stack(vjust = 0.5),colour="white",fontface = "bold")
base_size <- 20
p + ggtitle("CIBERSORT RESULTS") + 
theme_grey(base_size = base_size) + labs(x = "Samples", y = "",fontface = "bold") + 
scale_color_manual(values = c("#c90000",  "#ea9800",  "#04a327", "#6e0087")) + scale_fill_manual(values = c("#c90000",  "#ea9800",  "#04a327", "#6e0087"))
dev.off()



df <- read.table("MCP_DGIST.txt", sep="\t", header=TRUE)
df.m <- melt(df)

df_final <- cbind(df.m, percentage=c('9.24%','3.64%','0.18%','86.93%','28.05%','38.60%','0.49%','32.86%','58.43%','13.49%','2.20%','25.88%','11.17%','81.02%','0.87%','6.94%'))

pdf("MCP_DGIST.pdf", width=8, height=8)
p <- ggplot(df_final, aes(fill=X, y=value, x=variable))+geom_bar( stat="identity")+geom_text(aes(label = percentage),size =4.5, position = position_stack(vjust = 0.5),colour="white",fontface = "bold")
base_size <- 15
p + ggtitle("MCP counter RESULTS") + 
theme_grey(base_size = base_size) + labs(x = "Samples", y = "MCP score",fontface = "bold") + 
scale_color_manual(values = c("#c90000",  "#ea9800",  "#04a327", "#6e0087")) + scale_fill_manual(values = c("#c90000",  "#ea9800",  "#04a327", "#6e0087"))
dev.off()
...
B.cells.naive <- cbind(exp_real[,i],naiveB[,1])
colnames(B.cells.naive) <- c("DGIST","CIBERSORT")
corr <- cor(B.cells.naive)
